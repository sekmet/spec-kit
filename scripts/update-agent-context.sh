...
# Supports: CLAUDE.md, GEMINI.md, .github/copilot-instructions.md and .windsurf/rules/cascade-instructions.md
...
CASCADE_FILE="$REPO_ROOT/.windsurf/rules/cascade-instructions.md"
...
case "$AGENT_TYPE" in
    ...
    "copilot")
        ...
        ;;
    "cascade")
        update_agent_file "$CASCADE_FILE" "Windsurf Cascade"
        ;;
    "")
        ...
        [ -f "$CASCADE_FILE" ] && update_agent_file "$CASCADE_FILE" "Windsurf Cascade"
        
        # If no files exist, create based on current directory or ask user
        if [ ! -f "$CLAUDE_FILE" ] && [ ! -f "$GEMINI_FILE" ] && [ ! -f "$COPILOT_FILE" ] && [ ! -f "$CASCADE_FILE" ]; then
...
echo ""
echo "Usage: $0 [claude|gemini|copilot|cascade]"
...
echo "  - cascade: Update only .windsurf/cascade-instructions.md"